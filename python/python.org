...
#+name: herepoly
#+BEGIN_SRC python :session py  :tangle herepoly.py :results output verbatim
  <<eprint>>

  <<featuregeometry>>

  def herepoly(file, layer, feature):
      import ast
      ...
      jsonstr = geom.ExportToJson()   # now have a string
      # we could use plain eval(), but the following is safer
      json = ast.literal_eval(jsonstr) # now have a dict
      if json['type'] != 'MultiPolygon':
          eprint("""type is '{0}', but we only support
          'MultiPolygon'""".format(json['type']))
          sys.exit(2)
      ...
      for pgon in range(len(json['coordinates'])):
          for lring in range(len(json['coordinates'][pgon])):
              # zip(*zip): http://stackoverflow.com/q/2233204/1527747 and
              # https://docs.python.org/3/tutorial/controlflow.html#unpacking-argument-lists
              # http://stackoverflow.com/a/2511319/1527747 ("it's (apply)")
              (x,y,z) = zip(*json['coordinates'][pgon][lring])
              ignore = ax.plot(x, y, color='#6699cc', alpha=0.7,
...
#+BEGIN_SRC python :tangle extractwkt.py
  ...
  def main(argv):
      ...
      parser.add_argument('-t', '--type', choices=['Wkt', 'Json'], default='Wkt',
