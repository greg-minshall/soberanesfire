...
#+name: bbox
#+BEGIN_SRC python :var lat=36.4 :var lon=-121.858 :var zoom=5 :var mapurl=MAPURL :var parms=parameters
  """given (LAT, LON), a set of coordinates, and ZOOM, a [proposed] zoom
  level, return the "bounding box" of tiles we want to fill the screen.
  we *also* return a (possibly) updated zoom level, in case the input
  zoom level was so small that it wouldn't support filling the whole
  screen
  ...
  def tilenos(lat, lon, zoom):
     """actually do the work"""
     import math

     tx,ty = deg2xy(lat, lon, zoom)

     # so, tx,ty is tile coordinates of the center of the map.  now get
     ...
     snw = (tx - ((swit+1)/2), ty - ((shit+1)/2))
     sse = (tx + ((swit+1)/2), ty + ((shit+1)/2))

     # check to see if we are zoomed too far out
     if snw[0] < 0 or snw[1] < 0:
        return tilenos(lat, lon, zoom+1)
     else:
        return (snw[0], snw[1], sse[0], sse[1], zoom, tx, ty)

  p = {}
  for i in parms:
     p[i[0]] = i[1]

  return tilenos(lat, lon, zoom)
#+END_SRC

#+RESULTS: bbox
| 3 | 10 | 7 | 14 | 5 | 5 | 12 |

#+CALL: bbox(lat=0, lon=0, zoom=0)
