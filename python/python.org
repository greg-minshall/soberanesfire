...
#+name: ppShPoly
#+BEGIN_SRC python :tangle ppshpoly :var parms=parameters :var mapurl=MAPURL :shebang "#!/usr/bin/env python"
  import os
  ...
  def main(argv):
      ...
      (centx, centy) = (shape.centroid.x, shape.centroid.y)

      # get image from getmapimage; returns image, plus the bbox (in lat/lon) of image
      zoom = 10
      image = getmapimage(centx, centy, zoom)
