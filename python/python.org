...
#+BEGIN_SRC python
  def shPolyShape(shape):
      """prints the *EXTERIOR* of SHAPE (using matplotlib)""" # XXX to interior cutouts
      # from https://github.com/matplotlib/matplotlib/issues/5836
      import warnings;
      with warnings.catch_warnings():
          warnings.simplefilter("ignore");
...
#+BEGIN_SRC python :session py  :tangle shpoly :results none :shebang "#!/usr/bin/env python"
  ...
  def main(argv):
      ...
      gp.add(shape)
      # a problem is that when this process terminates, the plot window
      # tends to disappear.  the following, in the case of running under
      # gp.py under python, leaves the window up (and still tracking
      # mouse movements).  in theory this *shouldn't* work, but however
      # python closes its subprocess seems to leave the plot working.
      # well, at least for term=x11.
      gp.write("pause mouse") # leave window running XXX
...
#+BEGIN_SRC python
  ...
  style spec
          ...
          # fillstyle
          if defs['fillstyle'] == "none":
              results += " fillstyle empty"
          else:
              results += " fillstyle"
              if defs['transparency'] != 1:
                  results += " transparent {s} density {t}".format(s=defs['fillstyle'],
                                                                   t=defs['transparency'])
              else:
                  results += " " + defs['fillstyle']
          # including border
          if defs['border'] == 'noborder':
              results += " noborder"
          else:
              results += " " + defs['border']
              # linestyle (only if a border)
              if {defs['linetype'], defs['linecolor'], defs['linewidth']} != set([""]):
                  # something isn't blank
                  results += ifel('linetype', defs['linetype'])
      ...
      def __add__(self, shape, **kwargs):
          ...
          if shape.type in ['MultiPoint', 'MultiLine', 
              ...
              for geom in shape.geoms:
                  self.__add__(geom)  # recurse for each sub-geometry
          ...
          #    pass
              ...
              # gp.py's write() call sleeps here with the comment
              # "gnuplot actions are nonblocking"
              # so, we short cut that here
      ...
      def add(self, shape, **kwargs):
          ...
          self.__add__(shape, **kwargs)
          gp.GP.write(self, "set xrange [{x}:{X}]".format(x=self.minx,
                                                          X=self.maxx))
          gp.GP.write(self, "set yrange [{y}:{Y}]".format(y=self.miny,
                                                          Y=self.maxy))
