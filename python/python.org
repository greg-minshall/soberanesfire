...
#+BEGIN_SRC python :results output verbatim
  ...
  def bbox(lat, lon, zoom):
     ...
     print lat, lon, zoom, " ==> ", tcenterx, tcentery
     ...
     print tnwx, tnwy, tsex, tsey, zoomexp
     tnwx, tnwy = tilexyfixup(tnwx, tnwy, zoom)
     tsex, tsey = tilexyfixup(tsex, tsey, zoom)
     print tnwx, tnwy, tsex, tsey, zoomexp
...
#+BEGIN_SRC python :results output verbatim :var lat=36.4 :var lon=-121.858 :var zoom=5 :var parms=parameters :var mapurl=MAPURL
  ...
  print tnwx, tnwy, tsex, tsey, 2**zoom
  imx = 0
  for tx in map(lambda x: (x+tnwx)%(2**zoom), range(swit)):
     imy = 0
     for ty in map(lambda y: (y+tnwy)%(2**zoom), range(shit)):
        print tx, ty
