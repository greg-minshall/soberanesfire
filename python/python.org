...
#+BEGIN_SRC python :results output verbatim
  ...
  class TileBbox:                    # a Tile Bounding Box
    ...
    this box
      ...
      @staticmethod
      def __deg2tilexy__(lat_deg, lon_deg, zoom):
          """actually do the work of deg2tilexy"""
  ...
  coordinates of the required tile.  we check for numerical instability
  by searching around the passed point and making sure everything "close enough" ends up in the same tile"""
          ...
          for i, j in zip([-xdelta, xdelta], [-ydelta, ydelta]):
              print i,j
              results.add(TileBbox.__deg2tilexy__(lat_deg+i, lon_deg+j, zoom))

          if len(results) != 1:
              print results
              raise ArithmeticError("deg2tilexy on point ({0},{1}) too close to tile border at zoom {2}".format(lat_deg, lon_deg, zoom))

          lat_rad, lon_rad = results.pop()
...
: (168.75, -180.0, 5) (2.9452431127404313, -3.141592653589793) (0, 0)
: (0, 1) (168.75, -180.0) (0, 0)
