...
#+BEGIN_SRC python :var kmlfiles=filelist :var trigger="Heat Perimeter" :var select="Polygon" :results output verbatim :tangle program.py
  ...
  def fprolog():
      return "{0}\n{1}\n{2}\n{3}".format(fprolog1, fprolog2(), fprolog3(), fprolog4())
  ...
  def kmlfrags():
      ...
      result = re.sub("<FRAGID>", "0", pprolog1)
      result = result+ppost
      if kmls != []:
          nkmls = len(kmls)
          ncolors = len(colors)
          ...
          for kml in kmls:
              i = it.next()
              print kmlfiles[i]
              result = result+re.sub("<POLYGON>", kml, 
                                     re.sub("<FRAGID>", str(i), pprolog1))
              # XXX insert actual data
              result = result+ppost
      result = result+re.sub("<FRAGID>", "N", pprolog1)
      result = result+ppost
      return result
