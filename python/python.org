...
a good simple reference for xpath expressions at [[http://www.w3schools.com/xsl/xpath_syntax.asp][w3schools]]
...
#+BEGIN_SRC python :tangle xpathusing.py :results output verbatim
  ...
  def extract_kmls():
        ...
        for file in kmlfiles:
            ...
            kmls = kmls + [etree.tostring(tree.xpath(expr, namespaces=nspace)[0])]
        return kmls
  ...
  def main(argv):
      ...
      def_expr = '//kml2.2:MultiGeometry[../kml2.2:name="Heat Perimeter"] | //kml2.2:MultiGeometry[../../kml2.2:name="Fire Perimeter"]'

      # XXX
      def_input = ["/Users/minshall/work/gis/play/IR/20160729_Soberanes_IR/doc.kml",
      ...
      # use "type=str" since we use the file name for ogr.Open()
      parser.add_argument('kmlfiles', type=str, nargs="?", default=def_input)
      args = parser.parse_args();
      kmlfiles = args.kmlfiles
      nspace = "{{{0}:{1}}}".format(args.nsabbrev, args.nslong)
      layername = args.layername
      expr = args.expr
