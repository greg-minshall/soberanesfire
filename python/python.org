...
#+BEGIN_SRC python
  ...
  style spec
      ...
      def add(self, shape, **kwargs):
          ...
          # still tracking mouse movements).  in theory this *shouldn't*
          # work, but in whatever way python closes its subprocess, it
          # seems to leave the plot working.  well, at least for
          # term=x11.
          gp.GP.write(self, "pause mouse") # leave window running XXX
#+END_SRC
