...
#+BEGIN_SRC python :results output verbatim
  ...
  class TileBbox:                    # a Tile Bounding Box
    ...
    of this box
          ...
          """
          return self.docalc(self.tpt.x, self.tpt.y)
    ...
    this box
          ...
          """
          se = self.tpt+(self.wit, self.hit)
          return self.docalc(self.tpt.x+(self.wit+1), self.tpt.y+(self.hit+1)) # XXX
...
#+BEGIN_SRC python :tangle ppshpolymap :var parms=parameters :var mapurl=MAPURL :shebang "#!/usr/bin/env python"
  ...
  def main(argv):
      ...
      pp.addimage(image, extent=[dnwlon, dselon, dnwlat, dselat])
      pp.plot()
...
#+BEGIN_SRC python
  ...
  def calcbytbbox(nwlat, nwlon, zoom):
      global hwit

      tbbox = TileBbox(nwlat, nwlon, zoom)
...
#+BEGIN_SRC python :var zoom=32 :var nwlon=-146.25 :var nwlat=55.7765730187 :var parms=parameters :results output verbatim
  <<testcalcs>>
  hwit = None
  p = {}
  for i in parms:
  ...
  print "nwlat, nwlon, hwit: ", (nwlat, nwlon, hwit)
  print "diff by tbbox: ", bytbbox, diffs(bytbbox)
  print "diff by algorithm: ", byalgo, diffs(byalgo)
#+END_SRC

#+RESULTS:
: perytile, perxtile:  3.96050047129e-08 8.38190317154e-08
: nwlat, nwlon, hwit:  (55.7765730187, -146.25, 5)
: diff by tbbox:  (55.776573160092575, -146.25000016763806, 55.77657287724282, -146.24999966472387) (2.8284975428505277e-07, -5.029141902923584e-07)
: diff by algorithm:  (55.7765730187, -146.25, 55.77657282067498, -146.24999958090484) (1.9802502038146486e-07, -4.1909515857696533e-07)
