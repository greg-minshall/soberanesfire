...
#+BEGIN_SRC python :results output verbatim
  ...
  class TBBox:                    # a Tile Bounding Box
     ...
     def docalcx(tilex, zoom):
        """calculate the x-coordinate in *data* space of *tile* space TILEX"""
        return (tilex/(2**zoom)*360)-180
     def docalcy(tiley, zoom):
        """calculate the y-coordinate in *data* space of *tile* space TILEY"""
        return (180/math.pi)*math.arctan(math.sinh(math.pi-
                                                   ((y*2*math.pi)/(2**zoom))))
     def dnw(self):
        """return the xy coordinates, in *data* space, of the nw corner of
  this box

        """
        return (docalcx(self.tnwx, self.zoom),
                docalcy(self.tnwy, self.zoom))
      
     def dse(self):
        """return the xy coordinates, in *data* space, of the se corner of
  this box

        """
        return (docalcx(self.tsex, self.zoom),
                docalcy(self.tsey, self.zoom))

  def tbbox(lat, lon, zoom):
     ...
     return TBBox(tnwx, tnwy, wit, hit, zoom)
#+END_SRC
...
#+BEGIN_SRC python :tangle ppshpolymap :var parms=parameters :var mapurl=MAPURL :shebang "#!/usr/bin/env python"
  ...
  def main(argv):
      ...
      zoom = 0
      (image, nwx, nwy) = getmapimage(lat, lon, zoom)
