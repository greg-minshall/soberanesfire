...
#+BEGIN_SRC python
  ...
  class GpShape(gp.GP):
      ...
      # i don't want to duplicate all of gnuplot's parameters, as a
      # "power user" can always just to g.write("...").  but, some
      # things are fairly common.

      # typical parameters: fill; fillcolor; linewidth; linecolor;
      # linestyle; transparency

      # so, plan is to have defaults from "compile" time, the ability to
      # change the defaults (for subsequent calls), and the ability to
      # specify a different value on each invocation of add)
  ...
  specified as a single string (e.g., linecolor="rgbcolor 0x000000")
      ...
      def add(self, shape, **kwargs):
          ...
          #    pass
              ...
              gp.GP.write(self, "set yrange [{y}:{Y}]".format(y=self.miny,
                                                              Y=self.maxy))
              gp.GP.write(self, "set object {i} {o}".format(i=self.objid,
                                                           o=self.__stylespec__(**kwargs)))
