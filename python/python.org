...
[[http://matplotlib.org/basemap/][basemap]] and [[http://scitools.org.uk/cartopy/][cartopy]].
...
| nw | 36 34'36.00 N | 122 deg 04'13.08 W |
| se | 36 09'00.00 N | 121 deg 23'51.80 W |
...
#+BEGIN_SRC python
  def deg2latlon(*args):
      ...
      for istr in args:
          ...
          if matched == None:
              raise ValueError("invalid lat/lon spec '{0}'".format(input))
...
#+BEGIN_SRC python :results output :var input=cornersindegrees :results output
  <<deg2latlon>>
  print input
  print deg2latlon(input[0][2])
#+END_SRC

#+RESULTS:

  corners = {input[0][0]: deg2latlon(input[0][1], input[0][2]),
             input[1][0]: deg2latlon(input[1][1], input[1][2])}
