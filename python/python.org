...
#+BEGIN_SRC python :session py  :tangle herepoly.py :results none :shebang "#!/usr/bin/env python"
  ...
  def herepoly(ijson):
      ...
      print("{0} before warnings".format(os.times()))
      import warnings;
      with warnings.catch_warnings():
          warnings.simplefilter("ignore");
          print("{0} before import matplotlib".format(os.times()))
          from matplotlib import pyplot
          print("{0} after import matplotlib".format(os.times()))
      import sys
      ...
      if ijson['type'] == 'MultiPolygon':
          for pgon in range(len(ijson['coordinates'])):
              for lring in range(len(ijson['coordinates'][pgon])):
                  ...
                  print("{0} before getxy".format(os.times()))
                  (x,y) = getxy(*ijson['coordinates'][pgon][lring])
                  print("{0} before ax.plot".format(os.times()))
  ...
  def main(argv):
      ...
      print("{0} before extractjson".format(os.times()))
      json = extractjson(args.filename[0], args.layername, args.featurename)
      print("{0} after extractjson".format(os.times()))
...
#+BEGIN_SRC python :tangle extractwkt.py :shebang "#!/usr/bin/env python"
  <<caution>>
...
#+BEGIN_SRC python :tangle differencewkt.py :shebang "#!/usr/bin/env python"
  <<caution>>
...
#+BEGIN_SRC python :tangle awklike.py :shebang "#!/usr/bin/env python"
  <<caution>>
...
#+BEGIN_SRC python :tangle xpathusing.py :results output verbatim :shebang "#!/usr/bin/env python"
  <<caution>>
...
#+BEGIN_EXAMPLE
bash greg-minshalls-mbp-2: {1308} ./herepoly.py -l OGRGeoJSON -f "Heat Perimeter" x.json 
...
a couple of packages.  Gnuplot (fink: gnuplot-py27) seems nice.  otoh,
