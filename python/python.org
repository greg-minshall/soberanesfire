...
#+BEGIN_SRC python
  ...
  def bbox(lat, lon, zoom):
     ...
     tnwx, tnwy = (tcenterx - ((swit+1)/2), tcentery - ((shit+1)/2))
     tsex, tsey = (tcenterx + ((swit+1)/2), tcentery + ((shit+1)/2))

     zoomexp = 2**zoom            # just for convenience
     # check to see if we are zoomed too far out, wrapped around, or some such
     print swit, shit, tnwx, tnwy, tsex, tsey, 2**zoom
     if tnwy < 0:                 # we've wrapped over the north pole
        # (tnwy % shit) is how far we are from north pole, so go that
        # far from it on *this* side
        tnwy = tnwy % shit
        tnwx += zoomexp/2         # but, on the opposite side of the world
     if tsey >= shit:             # we've wrapped under the south pole
        # here, want to be (tsey%shit) away from south pole (max)
        tsey = shit - (tsey % shit)
        tsex += zoomexp/2         # but, again, around the world
     if tnwx < 0:        # we've wrapped around to the west
        # (twnx % swit) is how far we are from 0
        tnwx = swit - (tnwx % swit)
     if tsex >= swit:    # we've wrapped around to the east
        tsex = tsex % swit

     return (tnwx, tnwy, tsex, tsey, zoom)
#+END_SRC

#+BEGIN_SRC python :results output verbatim :var lat=90 :var lon=0 :var zoom=0 :var parms=parameters
  <<bbox>>
  p = {}
  for i in parms:
     p[i[0]] = i[1]

  print bbox(lat, lon, zoom)
#+END_SRC
...
#+call: getpngs(zoom=0, lat=0, lon=180)
