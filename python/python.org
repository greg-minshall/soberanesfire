...
#+name: fcomplete
#+BEGIN_SRC python :var kmlfiles=filelist :var trigger="Heat Perimeter" :var select="Polygon" :results output verbatim :tangle program.py
  import sys
  ...
  def fprolog():
      ...
      return "{0}\n{1}\n{2}\n{3}".format(re.sub("<DOCNAME>", "Soberanes", fprolog1),
  ...
  def kmlit(label, kml, fname):
      ...
      if kml == None:
          ...
          if type(label) is int and label != 0:
              label = int(math.ceil((1.0*ncolors/nkmls)*label))
          return re.sub("<POLYGON>", kml,
                        re.sub("<FRAGID>", str(label), 
                        re.sub("<PLACEMARKNAME>", fname[0], pprolog1))) + ppost


  def kmlfrags():
      global nkmls

      kmls = extract_kmls(list(reversed(kmlfiles)), trigger, select)
      kml0 = kmls[0]
      kmlN = kmls[len(kmls)-1]
      kmls = kmls[1:-1]           # get rid of first and last
      nkmls = len(kmls)
      result = kmlit(0, kml0, kmlfiles[0])
      if kmls != []:
          it = iter(range(nkmls))
          for kml in kmls:
              i = it.next()
              if kml != None:
                  result = result+kmlit(i, kml, kmlfiles[i])
      result = result+kmlit("N", kmlN, kmlfiles[len(kmlfiles)-1])
      return result
