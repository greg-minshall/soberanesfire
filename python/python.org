...
#+BEGIN_SRC python :var lat=36.4 :var lon=-121.858 :var zoom=5 :var mapurl=MAPURL
  <<tbbox>>
  x,y = TBBox.deg2tilexy(lat, lon, zoom)
...
#+name: tbbox
#+BEGIN_SRC python :results output verbatim
  class TBBox:                    # a Tile Bounding Box
      def __init__(self, lat, lon, zoom):
          self.__tbbox__(lat, lon, zoom)
      ...
      def docalc(self, tilex, tiley):
          """calculate the xy-coordinates in *data* space of *tile* space
    (TILEX, TILEY)"""
          import math
          lon_deg = (((tilex*1.0)/(2.0**self.zoom))*360.0)-180.0
          lat_rad = math.atan(math.sinh(math.pi-((tiley*2*math.pi)/(2.0**self.zoom))))
    ...
    of this box
          ...
          """
          return self.docalc(self.tnwx, self.tnwy)
    ...
    this box
          ...
          """
          return self.docalc(self.tnwx+self.wit, self.tnwy+self.hit) # XXX
      ...
      def deg2tilexy(self, lat_deg, lon_deg, zoom):
          ...
          return self.tilexyfixup(xtile, ytile, zoom)

      def __tbbox__(self, lat, lon, zoom):
...
#+BEGIN_SRC python :results output verbatim :var lat=36.4 :var lon=-121.858 :var zoom=5 :var parms=parameters :var mapurl=MAPURL
  ...
  image.
     ...
     tbox = TBBox(lat, lon, zoom)
     print "tbbox tnwx, tnwy, wit, hit, zoom: ", tbox.tnwx, tbox.tnwy, tbox.wit, tbox.hit, tbox.zoom
...
2 1
2 1
<__main__.Tbbox instance at 0x10c31ad88>
#+end_example
  ...
  possible.
      ...
      def __add__(self, i,j):
          ...
          if i < 0:
              ...
              i = self.powzoom+i
          if j < 0:
              j = self.powzoom+j  # ditto

          # okay, at most once (-1) around the world
          i = i % self.powzoom
          j = j % self.doubles    # here, we might go a bit further

          self.x += i             # longitude trivial (we do modulus
          ...
          # this.
          if self.__sameside__(self.y, self.y+j):
              self.y += j         # easy case, on same side, just add
          else:
              self.x += self.halves    # other side of world
              # next, get to the first tile beyond S pole ...
              j -= (self.powzoom-self.y)
              self.y = self.powzoom-1; # .. a tile called this
              self.y -= j         # then, "advance backwards" the rest
                                  # of our journey
...
#+BEGIN_SRC python :tangle ppshpolymap :var parms=parameters :var mapurl=MAPURL :shebang "#!/usr/bin/env python"
  ...
  def main(argv):
      ...
      # get image from getmapimage; returns image, plus the tbbox (in lat/lon) of image
      zoom = 0
      (image, (dnwlat, dnwlon), (dselat, dselon)) = getmapimage(lat, lon, zoom)
      ...
      pp.addimage(image, extent=[dnwlon, dnwlat, dselon, dselat])
      pp.plot()
