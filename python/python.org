...
#+BEGIN_SRC python
  ...
  style list
          ...
          # style list.  someday may want to scan the list to go from
          # "generic" to "gnuplot".  but, we don't need that yet.

          results = self.__rewrite__(defs, " ", " ")
          print results
          return results
