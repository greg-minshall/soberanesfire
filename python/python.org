...
#+BEGIN_SRC python :session py  :tangle herepoly.py :results none :shebang "#!/usr/bin/env python"
  ...
  def herepoly(ijson):
      ...
      if ijson['type'] not in ['MultiPolygon', 'Polygon']:
          eprint("type is '{0}', but we only support 'MultiPolygon' and 'Polygon'".format(ijson['type']))
          sys.exit(2)
      ...
      for pgon in range(len(ijson['coordinates'])):
          for lring in range(len(ijson['coordinates'][pgon])):
              ...
              (x,y,z) = zip(*ijson['coordinates'][pgon][lring])
  ...
  def main(argv):
      ...
      geom = featuregeometry(args.filename[0], args.layername, args.featurename)
      ijsonstr = geom.ExportToJson()   # now have a string
      # we could use plain eval(), but the following is safer
      ijson = json.loads(ijsonstr) # now have a dict
